    ...
    if (in_array($file_type, $allowed_types)) {
        ...
        if (move_uploaded_file($file_tmp, $upload_path)) {
            ...
            if (mysqli_query($conn, $sql)) {
                header("Location: admin_dashboard.php?success=Profile picture updated successfully.");
                exit();
            } else {
...
if (isset($_POST['name']) && !empty($_POST['name'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        header("Location: admin_dashboard.php?success=Name updated successfully.");
        exit();
    } else {
...
if (isset($_POST['username']) && !empty($_POST['username'])) {
    ...
    if (mysqli_num_rows($result) == 0) {
        ...
        if (mysqli_query($conn, $sql)) {
            header("Location: admin_dashboard.php?success=Username updated successfully.");
            exit();
        } else {
...
if (isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
    ...
    if ($new_password === $confirm_password) {
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            header("Location: admin_dashboard.php");  // Stay on the same page if there's an error
            exit();
        }
    } else {
        $_SESSION['error'] = "Passwords do not match.";
        header("Location: admin_dashboard.php");  // Stay on the same page if passwords don't match
        exit();
    }
}
